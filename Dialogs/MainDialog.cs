...
namespace Barin.RomoteAssembly.Dialogs
{
    public class MainDialog : ComponentDialog
    {
        ...
        {
            if (!luis.IsConfigured)
            {
                await stepContext.Context.SendActivityAsync(
                    MessageFactory.Text("NOTE: LUIS is not configured. " +
                    "To enable all capabilities, add 'LuisAppId', 'LuisAPIKey' " +
                    "and 'LuisAPIHostName' to the appsettings.json file.",
            ...
            WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            if (!luis.IsConfigured)
            {
                ...
                // with an empty BookingDetailsInstance.
            ...
            var luisResult = await luis.RecognizeAsync<FlightBooking>(
